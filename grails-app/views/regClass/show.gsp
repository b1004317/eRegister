	...
	<body>
		...
		<div id="show-regClass" class="content scaffold-show" role="main">
			...
			<ol class="property-list regClass">
				...
				<g:if test="${regClassInstance?.classes_registrationsheets}">
				<li class="fieldcontain">
					<span id="classes_registrationsheets-label" class="property-label"><g:message code="regClass.classes_registrationsheets.label" default="Classesregistrationsheets" /></span>
					
						<g:each in="${regClassInstance.classes_registrationsheets}" var="c">
						<span class="property-value" aria-labelledby="classes_registrationsheets-label"><g:link controller="registrationSheet" action="show" id="${c.id}">${c?.encodeAsHTML()}</g:link></span>
						</g:each>
